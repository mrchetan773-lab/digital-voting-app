...
// ─────────────────────────────────────────────
// DATA GENERATION — 2050 Life Members
// ─────────────────────────────────────────────
...
// IN-MEMORY STATE
// ─────────────────────────────────────────────
let votes = { rural: {}, urban: {} };
let votedRuralMembers = new Set();
let votedUrbanMembers = new Set();
...
app.post('/api/auth/id', (req, res) => {
  ...
  if (votedRuralMembers.has(member.id) && votedUrbanMembers.has(member.id)) {
  ...
  if (adminSettings.ringcaptchaAppKey && adminSettings.ringcaptchaApiKey && token && token !== 'mock-token') {
    try {
      ...
      if (data.status === 'SUCCESS') {
        ...
        const hasMissing = !member.email || !member.address;
        return res.json({ member, hasMissing });
      } else {
...
app.delete('/api/admin/members/:id', adminAuth, (req, res) => {
  ...
  if (votedMembers.has(members[idx].id)) {
...
app.post('/api/admin/reset-votes', adminAuth, (req, res) => {
  votes = { rural: {}, urban: {} };
  votedMembers.clear();
...
// ── Submit Rural Vote ──
app.post('/api/vote/rural', (req, res) => {
  ...
  if (votedRuralMembers.has(memberId)) {
  ...
  // Tally votes
  rural.forEach(cid => { votes.rural[cid] = (votes.rural[cid] || 0) + 1; });
  votedRuralMembers.add(memberId);
...
// ── Submit Urban Vote ──
app.post('/api/vote/urban', (req, res) => {
  ...
  if (votedUrbanMembers.has(memberId)) {
  ...
  // Tally votes
  urban.forEach(cid => { votes.urban[cid] = (votes.urban[cid] || 0) + 1; });
  votedUrbanMembers.add(memberId);
...
app.get('/api/results', adminAuth, (req, res) => {
  const ruralResults = ruralCandidates.map(c => ({
    ...c, votes: votes.rural[c.id] || 0
  })).sort((a, b) => b.votes - a.votes);

  const urbanResults = urbanCandidates.map(c => ({
    ...c, votes: votes.urban[c.id] || 0
  })).sort((a, b) => b.votes - a.votes);

  // Let's create a union set for unique voters who voted in at least one
  const uniqueVoters = new Set([...votedRuralMembers, ...votedUrbanMembers]);
